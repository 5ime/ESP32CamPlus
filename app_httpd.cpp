...
static void draw_face_boxes(fb_data_t *fb, std::list<dl::detect::result_t> *results, int face_id) {
    ...
    for (auto prediction = results->begin(); prediction != results->end(); prediction++) {
        ...
        for (int j = 0; j < 10; j += 2) {
            int x0 = (int)prediction->keypoint[j];
            int y0 = (int)prediction->keypoint[j + 1];
...
static esp_err_t capture_handler(httpd_req_t *req) {
    ...
    if (fb->format == PIXFORMAT_RGB565
        ...
        auto &results = s2.infer((uint16_t *)fb->buf, {(int)fb->height, (int)fb->width, 3}, candidates);
#else
        HumanFaceDetectMSR01 s1(0.3F, 0.5F, 10, 0.2F);
        auto &results = s1.infer((uint16_t *)fb->buf, {(int)fb->height, (int)fb->width, 3});
#endif
        if (results.size() > 0) {
            fb_data_t rfb;
            rfb.width = fb->width;
        ...
        HumanFaceDetectMSR01 s1(0.1F, 0.5F, 10, 0.2F);
        HumanFaceDetectMNP01 s2(0.5F, 0.3F, 5);
        auto &candidates = s1.infer((uint8_t *)out_buf, {(int)out_height, (int)out_width, 3});
        auto &results = s2.infer((uint8_t *)out_buf, {(int)out_height, (int)out_width, 3}, candidates);
#else
        HumanFaceDetectMSR01 s1(0.3F, 0.5F, 10, 0.2F);
...
static void flash_burst(uint8_t level, uint16_t interval_ms, uint8_t repeat) {
    ...
    const uint8_t previous = app_get_flash_duty();
    for (uint8_t i = 0; i < repeat; ++i) {
        app_set_flash_duty(level);
